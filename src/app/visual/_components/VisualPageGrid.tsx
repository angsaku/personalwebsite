...
export default function VisualPageGrid({ items }: { items: VisualItem[] }) {
    ...
    <>
      ...
      {hasMore && (
        <div ref={sentinelRef} className="mt-12 flex justify-center">
          <div className="flex gap-1.5 items-center text-xs text-gray-700">
            <span className="w-1 h-1 rounded-full bg-gray-700 animate-pulse" />
            <span className="w-1 h-1 rounded-full bg-gray-700 animate-pulse [animation-delay:0.2s]" />
            <span className="w-1 h-1 rounded-full bg-gray-700 animate-pulse [animation-delay:0.4s]" />
          </div>
        </div>
      )}

      {!hasMore && items.length > BATCH && (
        <p className="mt-12 text-center text-xs text-gray-700">All {items.length} visuals loaded</p>
      )}
