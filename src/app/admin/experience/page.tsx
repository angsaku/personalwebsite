import Link from "next/link";
import { Plus } from "lucide-react";
import { createSupabaseServer } from "@/lib/supabase-server";
import ExperienceRow from "./_components/ExperienceRow";

export const revalidate = 0;

export default async function ExperiencePage() {
  const supabase = await createSupabaseServer();
  const { data: experiences } = await supabase
    .from("experiences")
    .select("id, company, role, period, location")
    .order("sort_order", { ascending: true });

  return (
    <div className="p-8">
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-2xl font-bold text-white">Work Experience</h1>
          <p className="text-sm text-gray-500 mt-1">
            {experiences?.length ?? 0} entries total
          </p>
        </div>
        <Link
          href="/admin/experience/new"
          className="flex items-center gap-2 bg-[#E5212E] text-white px-5 py-2.5 rounded-xl text-sm font-medium hover:bg-[#c41a25] transition-colors"
        >
          <Plus size={15} />
          New Experience
        </Link>
      </div>

      <div className="bg-[#0a1128] border border-white/[0.06] rounded-2xl overflow-hidden">
        <div className="grid grid-cols-12 gap-4 px-5 py-3 border-b border-white/[0.06]">
          <p className="col-span-4 text-xs text-gray-600 uppercase tracking-wide">Role / Company</p>
          <p className="col-span-3 text-xs text-gray-600 uppercase tracking-wide">Period</p>
          <p className="col-span-3 text-xs text-gray-600 uppercase tracking-wide">Location</p>
          <p className="col-span-2 text-xs text-gray-600 uppercase tracking-wide text-right">Actions</p>
        </div>

        {experiences && experiences.length > 0 ? (
          experiences.map((exp) => <ExperienceRow key={exp.id} exp={exp} />)
        ) : (
          <div className="py-16 text-center">
            <p className="text-gray-600 text-sm">No experience entries yet.</p>
            <Link
              href="/admin/experience/new"
              className="text-[#E5212E] text-sm mt-2 inline-block hover:underline"
            >
              Add your first experience →
            </Link>
          </div>
        )}
      </div>
    </div>
  );
}
