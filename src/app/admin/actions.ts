...
export async function upsertWorkflowConfig(formData: FormData) {
  const supabase = createSupabaseAdmin();
  const { data: existing } = await supabase.from("workflow_config").select("id").single();
  const payload = {
    headline: formData.get("headline") as string,
    philosophy: formData.get("philosophy") as string,
    engagement_content: formData.get("engagement_content") as string,
  };
  const { error } = existing
    ? await supabase.from("workflow_config").update(payload).eq("id", existing.id)
    : await supabase.from("workflow_config").insert(payload);
  if (error) return { error: error.message };
  revalidatePath("/approach");
  return { success: true };
}

export async function createWorkflowPrinciple(formData: FormData) {
  const supabase = createSupabaseAdmin();
  const { error } = await supabase.from("workflow_principles").insert({
    title: formData.get("title") as string,
    description: formData.get("description") as string,
    sort_order: Number(formData.get("sort_order") || 0),
  });
...
export async function updateWorkflowPrinciple(id: string, formData: FormData) {
  const supabase = createSupabaseAdmin();
  const { error } = await supabase.from("workflow_principles").update({
    title: formData.get("title") as string,
    description: formData.get("description") as string,
    sort_order: Number(formData.get("sort_order") || 0),
  }).eq("id", id);
...
export async function deleteWorkflowPrinciple(id: string) {
  const supabase = createSupabaseAdmin();
  await supabase.from("workflow_principles").delete().eq("id", id);
  revalidatePath("/approach");
  redirect("/admin/workflow");
}
...
export async function createWorkflowPhase(formData: FormData) {
  const supabase = createSupabaseAdmin();
  const { error } = await supabase.from("workflow_phases").insert({
    number: formData.get("number") as string,
    title: formData.get("title") as string,
    description: formData.get("description") as string,
  ...
  if (error) return { error: error.message };
  revalidatePath("/approach");
  redirect("/admin/workflow/phases");
}

export async function updateWorkflowPhase(id: string, formData: FormData) {
  const supabase = createSupabaseAdmin();
  const { error } = await supabase.from("workflow_phases").update({
    number: formData.get("number") as string,
    title: formData.get("title") as string,
    description: formData.get("description") as string,
    content: formData.get("content") as string,
...
export async function deleteWorkflowPhase(id: string) {
  const supabase = createSupabaseAdmin();
  await supabase.from("workflow_phases").delete().eq("id", id);
  revalidatePath("/approach");
  redirect("/admin/workflow/phases");
}

export async function createWorkflowService(formData: FormData) {
  const supabase = createSupabaseAdmin();
  const { error } = await supabase.from("workflow_services").insert({
...
export async function updateWorkflowService(id: string, formData: FormData) {
  const supabase = createSupabaseAdmin();
  const { error } = await supabase.from("workflow_services").update({
    service_name: formData.get("service_name") as string,
    content: formData.get("content") as string,
    sort_order: Number(formData.get("sort_order") || 0),
  }).eq("id", id);
