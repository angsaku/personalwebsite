      ...
      {value ? (
        <div className="relative group rounded-xl overflow-hidden border border-white/[0.08]">
          {isVideo(value) ? (
            ...
            <img src={value} alt="Preview" className="w-full aspect-video object-cover" />
          )}
        ...
        >
          {uploading ? (
            ...
            <>
              <div className="w-10 h-10 rounded-xl bg-white/[0.04] border border-white/[0.08] flex items-center justify-center gap-1">
                <ImageIcon size={14} className="text-gray-600" />
                <Film size={14} className="text-gray-600" />
              </div>
              <span className="text-xs text-gray-500">Click to upload</span>
              <span className="text-[11px] text-gray-700">PNG, JPG, WEBP, WEBM · Max 50MB</span>
            </>
          )}
        </button>
      )}
